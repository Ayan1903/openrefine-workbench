...
### 6. JaCoCo レポートを生成する（test + report）
...
- テスト実行中に `jacoco.exec` が生成されていれば、後から `jacoco:report` だけ再実行して `jacoco.xml` を再生成できます。
- 逆に `jacoco.exec` が無い場合は、レポート再生成はできません。

### 7. JaCoCo XML を XTDB に再投入する
