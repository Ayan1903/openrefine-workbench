...
(defn build-fix-prompt
  ...
  (let [bucket-lines (->> (:errors bucket)
    ...
    (str "以下の Java 生成テストを、コンパイルエラーの束をまとめて解消する形で修正してください。\n"
         ...
         "    List<Integer> documentIds = Arrays.asList(1, 2, 3);\n"
         "    ...\n"
         "  */ }\n"
         "- @Disabled でマークされたテストメソッドについて、メソッド本体全部（すべての assertion / when / verify など）を /* */ コメントで括ること。これにより、存在しないメソッド呼び出しのコンパイルエラーが消える\n"
         ...
         "  例: @Disabled(\"置換先メソッドが確定できない\") public void testXxx() { /* ... */ }\n"
         "- 削除推奨メソッドでも、元のコード（stub, verify, assertions含む）は削除せず、@Disabled で保持し、すべてコメント化すること\n"
